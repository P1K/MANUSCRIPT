...
It is easy to see that this is the highest possible minimum distance that can be achieved by a code defined with a
systematic generator matrix, as all rows of such a matrix have weight at most $n - k + 1$~; this is
...
There is no MDS code with symbols in $\Fu$ of length $n$ with $n > \#\Fu + 1$, (except degenerate cases\footnote{This maximal length can be increased by one
