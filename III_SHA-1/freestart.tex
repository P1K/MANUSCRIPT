...
in the case of \shaone. Note that unlike hash function attacks \emph{à la} Wang, our freestart collisiosn use only one block, which has some consequences building
the attack.
...
selectively changing the value of some bits of the message. As the structure of our attacks imples shifting down the free window of the message, changing a bit in this window may also impact
...
A slight downside of our meet-in-the-middle implementation, however, is that it did not give strict guarantees that the paths it produced were completely valid, as they may innclude contradictory conditions
...
For these steps JLCA would thus optimize for the wrong model, wich includes all variants.
