...
\begin{defi}[Preimage resistance] Given a target $k$, an adversary must not be able to find a message $m$ such that $\hash(m) = k$ with non-negligible probability using
less computational resources than what is required to compute $\hash$ $\bigo(2^n)$ times.
\label{def:pre}
\end{defi}
\begin{defi}[Second preimage resistance] Given a message $m$, an adversary must not be able to find a different message $m'$ such that
...
\label{def:2pre}
\end{defi}
\begin{defi}[Collision resistance] One must not be able to find two distinct ``fresh'' messages $m$ and $m'$ such that $\hash(m) =
...
The complexities associated with these definitions correspond to the ones of the best known generic algorithms that can find messages with the desired
...
\begin{enumerate}
\item $\mess$ is padded to a size multiple of the message \emph{block size} $b$. Various padding rules may be employed, but it is obviously important that they should not
introduce trivial collisions. Also, most of the time, the size (usually in bits) of the non-padded message $\mess$ is included in the padding in one way or the other.
...
\end{prop}
\begin{proof}
Let $\mess$ be a message fitting on $r+1$ blocks after padding s.t. $\hash(\mess) = k$, with $k$ the preimage target.
...
A good example of such a property is the concept of \emph{multicollision}, where one is required to find $r > 2$ messages $\mess_{0},\ldots,\mess_{r-1}$ whose images through $\hash$
...
of an \iv and a message such that $\hash_\freeiv(\messblock) = k$, with $\hash_\freeiv(\cdot)$ denoting
...
function calls~\cite{DBLP:conf/fse/Saarinen03}. Such pairs are made of two \ivs $\state_{0,\ldots,4}$, $\state'_{0,\ldots,4}$ and messages $\mess_{0,\ldots,15}$,
