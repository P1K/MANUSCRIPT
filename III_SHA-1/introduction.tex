...
\end{prop}
\begin{proof}
Let $m$ be a message fitting on $r+1$ blocks after padding s.t. $\hash(m) = k$, with $k$ the preimage target.
...
of an \iv and a message such that $\hash_\freeiv(m) = k$, with $\hash_\freeiv(\cdot)$ denoting
