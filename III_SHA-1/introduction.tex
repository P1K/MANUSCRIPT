...
\begin{defi}[Preimage resistance] Given a target $\targ$, an adversary must not be able to find a message $\messhash$ such that $\hash(\messhash) = \targ$ with non-negligible probability using
...
A good example of such a property is the concept of \emph{multicollision}, where one is required to find $r > 2$ messages $\mess_{0},\ldots,\mess_{r-1}$ which images through $\hash$
