...
\subsection{The differential path for most of the first two rounds}
...
be determined by linerisation of the step function, given the differences
in the message.
...
Finally, on the implementation side, we show how the neutral bits are packed together inside the inter-snippet buffers from \autoref{sec:part_sol} in \autoref{fig:nb_packing76} and
...
is the value of the thirteenth bit in some instance of $\expmess_{14}$, \ie $\expmess_{14}[13]$; the fact that this circle is the leftmost one in the top sequence means that in the buffer, this information is stored as the MSB
of the first word.
One should note that not all consecutive bits of a message are neutral; non-neutral bits do not need to be stored,
but it is nonetheless useful to maintain the relative distance between the actual neutral bits inside
the packing.
...
Although the separations between the 32-bit words are not materialised, they should be taken into account, and
