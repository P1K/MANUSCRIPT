...
On appelle
$\kappa$ la \emph{taille de clef} et $n$ la \emph{taille de bloc} de $\E$. Usuellement, ces paramètres on pour valeur
...
Si $\E$ est tel que la permutation $\E(k,\cdot)$ est difficile à inverse quand $k$ est inconnu, $A$ et $B$
pourrait s'attendre à ce qu'un canal de communication sûr consiste à injecter leurs messages
vers les chaînes
...
Même pour de petites valeurs de $n$, utiliser FYK requiert une quantité considérable d'aléas paramétrisé par les clefs,
...
et qui tourne en temps inférieur à
...
Il n'existe pas de réponse évident à ce que «\,raisonnable\,» et «\,non négligeable\,» signifient dans le contexte d'attaques réelles, notamment parce que les tailles de clefs
...
(le chiffre par bloc $\E$) est différente par rapport à une permutation tirée aléatoirement dans $\Pi_{2^n}$.
...
majeur de distingueurs statistique est celle des \emph{distingueurs linéaires}.

Soit un chiffre par bloc $\E$~; une \emph{différentielle} pour $\E$ est une paire
...
$p$, $c \defas \E(k,p)$, $p'$, $c' \defas \E(k,p')$ tel que $p - p' = \Delta$, $c - c' = \delta$. Quand la différence est sur $\mathbf{F}_2^n$,
...
Pour qu'un distingueur sur $\E$ soit utile, il faut donc que son EDP soit différente de $2^{-n}$. Si elle en est suffisamment éloignée
...
En définissant la sécurité PRP, nous étions satisfait de la notion de distingueur. Cependant, dans le cas d'attaques concrètes, l'objectif
final est idéalement de retrouver la clef inconnue utilisée par l'oracle.
Le contexte d'une attaque est aussi généralement différent de celui d'un jeu de sécurité PRP, car l'attaquant connaît généralement déjà
...
chiffre réduit à $d$ tours, sur lequel le distingueur est supposé être efficace. En revanche, si l'hypothèse est fausse,
...
On peut faire certains commentaires concernant cette procédure. 1) Le coût associé aux hypothèses répétées devant être faites sur la valeur d'une partie de la clef
...
Soit une cible $targ$, un adversaire ne doit pas être capable de trouver un message
...
\end{fdefi}
\begin{fdefi}[Résistance aux collisions] On ne doit pas être capable de trouver deux messages «\,frais\,» différents $\messhash$ et $\messhash'$ tels que$\hash(\messhash) =
...
s'appliquent aussi bien à n'importe lesquelles). En conséquence, ils sont généralement entièrement ignorés dans un contexte cryptographique, et ils ne rendent pas la
...
en collision n'est pas nécessaire, ou que seule les attaques en seconde préimage soient une menace. Toutefois, on attend généralement d'une fonction de hachage qu'elle puisse être utilisée
...
de hachage $\hash$. Ainsi, même si un adversaire devait trouver la liste des utilisateurs et de leurs mots de passe associés, il ne serait
...
utilisée (nous ne rentrerons toutefois pas dans le détail de la construction de bons schémas dans cette introduction).
...
L'idée principale de ces schémas est que le partie effectuant une signature publie en avance un certain nombre d'empreintes, tout en gardant
secrètes les entrées à la fonction de hachage les produisant. Signer un message consiste alors révéler certains de ces entrées~; ainsi, être capable
...
et on dispose généralement d'alternative plus rapide, comme les MACs dits polynomiaux
...
qu'il n'y en a pas sur la fonction de hachage. D'une certaine tout ceci est proche de l'objectif de construire un mode d'opération sûr
...
à une taille multiple de la taille de bloc $b$. Plusieurs méthodes peuvent être employées à ces fins, mais il est évidemment important
...
construire des collisions~; de tels points fixes sont facile à trouver pour certaines fonctions de compression populaires, comme celles de type
...
$\compress$. Les deux entrées du premier de ceux-ci étant différentes, la collision qu'ils définissent est non triviale.
\end{proof}
...
de hachage concrète. En revanche, même si une telle construction est sûre dans un tel modèle (dit de l'oracle aléatoire), il n'est pas
...
\emph{multicollision}, qui consiste à chercher $r > 2$ messages $\messhash_{0},\ldots,\messhash_{r-1}$ dont les images par $\hash$
sont toute égales.
...
L'idée de base utilisée dans cette attaque exploite le fait que des collisions pour une fonction \merkdam $\hash$ peut être chaînées
...
Dean~\cite{dean}, puis Kelsey et Schneier~\cite{DBLP:conf/eurocrypt/KelseyS05}
...
de fonctions \merkdam en pratique, et de telles fonctions sont de fait toujours utilisées, comme
\shatwo~\cite{Nist-SHA}.
...
Prenant un peu d'avance, nous verrons dans la section~\ref{sec:description} que l'\iv et les blocs de message  de la fonction
...
\ivs $\state_{0,\ldots4}$, $\state'_{0,\ldots,4}$ et messages $\mess_{0,\ldots,15}$,
...
qu'on attend qu'elles réalisent, à condition que la fonction de compression est «\,idéale\,» et qu'elles ne sont pas appelées un nombre trop important de fois par rapport à la taille de leurs paramètres.


\paragraph{Construction éponge.} La construction éponge a été introduite en 2007 par
Bertoni \etal~\cite{SpongeFunctions}.
Elle est quelque peu différente de la construction \merkdam, notamment parce qu'elle n'utiliser pas une fonction de compression
...
est naturellement décorrélée de la taille des sa fonction constitutrice. Ainsi, cette construction permet facilement de construire des fonctions de hachage à sortie
...
(un taux plus élevé donne des fonctions plus rapides) et la sécurité (une capacité plus élevés donne une fonction plus sûre).
...
\item La fonction de compression est construite à partir d'un chiffre par bloc ad hoc utilisé en mode \emph{Davies-Meyer} mode~: 
soit
