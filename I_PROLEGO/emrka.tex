...
$\IEM^r$ from an ideal cipher. This is illustrated for the original Even-Mansour scheme in \autoref{fig:em3}.

\begin{figure}[!htb]
\begin{center}
\input{../I_PROLEGO/em3.tex}
\caption{A related-key distinguisher on $\IEM^1$. Dashed lines represent datapaths where a difference is injected and not canceled.\label{fig:em3}}
\end{center}
\end{figure}
...
\caption{Related-key queries to \proestotr with predictible output difference.\label{fig:otr21}}
\end{center}
\end{figure}
...
\caption{Related-key queries to \proestotr with unpredictible output difference.\label{fig:otr22}}
\end{center}
\end{figure}
...
for a 64-bit toy cipher in the \autoref{sec:toy}.
...
careful when contemplating such a decision for Even-Mansour ciphers (and FX constructions, to some extent), as in that case it is actually
